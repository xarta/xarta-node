...
BLUEPRINTS_INTERESTS_UID="${BLUEPRINTS_INTERESTS_UID:-10000}"
BLUEPRINTS_INTERESTS_GID="${BLUEPRINTS_INTERESTS_GID:-1000}"
...
chmod 2770 "$INTERESTS_DIR"
echo -e "    ${CYAN}ownership${NC}: $BLUEPRINTS_INTERESTS_UID:$BLUEPRINTS_INTERESTS_GID → $INTERESTS_DIR"
for category in "${INTEREST_CATEGORIES[@]}"; do
    ...
    chown "$BLUEPRINTS_INTERESTS_UID:$BLUEPRINTS_INTERESTS_GID" "$category_dir"
    chown "$BLUEPRINTS_INTERESTS_UID:$BLUEPRINTS_INTERESTS_GID" "$marker_dir"
    interest_stignore_text "$category" > "$ignore_tmp"
    if [[ ! -f "$ignore_file" ]] || ! cmp -s "$ignore_tmp" "$ignore_file"; then
        cp "$ignore_tmp" "$ignore_file"
    fi
    rm -f "$ignore_tmp"
    chmod 660 "$ignore_file"
    chown "$BLUEPRINTS_INTERESTS_UID:$BLUEPRINTS_INTERESTS_GID" "$ignore_file"
done
