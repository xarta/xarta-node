...
def check_gui_contract(gui_root: Path) -> None:
    ...
    require("chosen?.dispatch" in execute_action, "help action executor does not require dispatch metadata")
    ...
    require("mode: 'stream'" in help_js or 'mode: "stream"' in help_js, "help TTS playback is not configured for streaming")

    require("/api/v1/docs/search/explain" in docs_search_js, "Docs Search explain endpoint is not wired")
    require("data.display" in docs_search_js, "Docs Search explain path does not require display metadata")
    require("display.summary" in docs_search_js, "Docs Search explain summary is not rendered from display")
...
def check_api_contract(client: httpx.Client, query: str) -> None:
    ...
    require(short.get("playback_transport") == "streaming_tts", "short_response does not advertise streaming TTS")
    require(short.get("length_is_tts_limit") is False, "short_response incorrectly marks length as a TTS limit")
    require(short.get("length_reason") == "conversational_brevity", "short_response length reason is unclear")
    require("`" not in text and not re.search(r"https?://|\[(?:S|s)\d+\]", text), "short_response is not TTS-shaped")

    action = turn.get("action")
    require(isinstance(turn.get("action_catalog"), dict), "/api/v1/help/turn missing action_catalog")
    if action is not None:
        require(isinstance(action, dict), "help action is not an object")
        require(isinstance(action.get("dispatch"), dict), "cataloged help action missing dispatch metadata")
    ...
    require(isinstance(display.get("summary"), str) and display["summary"].strip(), "display.summary is empty")
...
def main() -> int:
    ...
    parser.add_argument("--gui-root", default=os.getenv("BLUEPRINTS_GUI_ROOT", "/xarta-node/gui-fallback"))
