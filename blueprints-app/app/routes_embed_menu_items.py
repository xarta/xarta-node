...
GET    /api/v1/embed-menu-items/config     → sanitized selector pages payload
...
async def list_embed_menu_items() -> list[EmbedMenuItemOut]:
...
async def seed_embed_menu_items() -> dict:
    ...
    with get_conn() as conn:
        for item in _DEFAULT_SEED:
            exists = conn.execute(
                "SELECT item_id FROM embed_menu_items WHERE item_key=?",
                (item["item_key"],),
            ).fetchone()
            if exists:
                skipped += 1
