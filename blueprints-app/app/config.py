...
if not NODE_ID:
    raise RuntimeError("BLUEPRINTS_NODE_ID is not set — add it to .env before starting the app")
...
PEER_SYNC_URLS: dict[str, list[str]] = {
    n["node_id"]: _peer_sync_urls(n, _self_node) for n in _peer_nodes
}
...
CORS_ORIGINS: list[str] = [f"https://{n['primary_hostname']}" for n in _active_nodes] + [
...
def _load_lxc841_dockge_file_config() -> dict:
    ...
    try:
        ...
        log.warning(
            "could not load LXC841_DOCKGE_CONFIG_FILE=%r: %s", LXC841_DOCKGE_CONFIG_FILE, exc
        )
        return {}
